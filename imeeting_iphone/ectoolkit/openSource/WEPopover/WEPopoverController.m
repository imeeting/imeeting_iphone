...
- (void)presentPopoverFromBarButtonItem:(UIBarButtonItem *)item 
                               withType:(navigationItemType)type 
                       inNavigationView:(UIView *)theView
               permittedArrowDirections:(UIPopoverArrowDirection)arrowDirections 
                               animated:(BOOL)animated{
    // define menu shown rectangle and show in navigation view
    CGRect _rect;
    
    // judge navigation item type
    switch (type) {
        case left:
            _rect = CGRectMake(0.0, 0.0, 50.0, 57.0);
            break;
            
        case right:
            _rect = CGRectMake([UIScreen mainScreen].bounds.size.width, 0.0, 50.0, 57.0);
            break;
    }
    
    // call method:(void) presentPopoverFromRect: inView: permittedArrowDirections: animated:
    [self presentPopoverFromRect:_rect inView:theView permittedArrowDirections:arrowDirections animated:animated];
}
